  ...
  return (
    <AppContainer>
      <WrapInLoader
        isReady={!!active}
        builder={() => (
          <div data-cy='TeachingSummaryScreen'>
            <Typography variant='h2' data-cy='TSTitle'>
              {active?.teaching.title}
            </Typography>
            <Typography variant='body2' data-cy='TSSubtitle'>
              {active?.teaching.subtitle}
            </Typography>
            <MenuList>
              {active?.teaching.chapters.map((chapter, i) => (
                <ChapterCard key={i} i={i} chapter={chapter} isDone={active.isChapterDone(i)} />
              ))}
            </MenuList>
          </div>
        )}
  ...
  return (
    <Link to={`chapter/${i}`}>
      ...
      <MenuItem>
        ...
        {isDone && <Check color='primary' data-cy={`DoneIcon:${i}`} />}
      </MenuItem>
    </Link>
  );
}
